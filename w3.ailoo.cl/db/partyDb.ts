...
export async function getCustomerPointsAccountBalance(partyId: number) {
    const connection : PoolConnection = await pool.getConnection();

    try {
        const [rows] = await connection.execute<RowDataPacket[]>(
            `
                select ca.*, sum(AmountPrice) as Balance
                from customeraccount ca
                         join CustomerAccountMovement cam on ca.Id = cam.CustomerAccountId
                where ca.Type = 0
                  and ca.PartyId in (?)
...
export const listAddressesByOrders = async (partyId: number) => {
    const connection = await pool.getConnection();
