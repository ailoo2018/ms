...
export class Price {
  private finalPrice: { amount: number };
  get thruQuantity(): number {
    return this._thruQuantity;
  }

  set thruQuantity(value: number) {
    this._thruQuantity = value;
  }
  private _thruQuantity: number;
  get fromQuantity(): number {
    return this._fromQuantity;
  }

  set fromQuantity(value: number) {
    this._fromQuantity = value;
  }
  private _fromQuantity: number;
  get domainId(): number {
    return this._domainId;
  }

  set domainId(value: number) {
    this._domainId = value;
  }
  private _domainId: number;
  get priceComponents(): any[] {
    return this._priceComponents;
  }

  set priceComponents(value: any[]) {
    this._priceComponents = value;
  }
  private _priceComponents: any[];
  get saleTypeId(): any {
    return this._saleTypeId;
  }

  set saleTypeId(value: any) {
    this._saleTypeId = value;
  }
  private _saleTypeId: any;
  get currency(): any {
  ...
  constructor(currency: any, saleTypeId: any) {
    ...
    this._priceComponents = [];
    this._domainId = 0;
    this._fromQuantity = 0;
    this._thruQuantity = Number.MAX_VALUE;
    this.finalPrice = {amount: 0};
  }

  get hasBasePrice() {
    return this._priceComponents.some(pc =>
    ...
    if (basePrice) {
      ...
      if (discount) {
        if (discount.percent > 0) {
          ...
          return new Money(roundedAmount, basePrice.price._currency);
        } else {
          const finalAmount = basePrice.price.amount - discount.price.amount;
          return new Money(finalAmount, basePrice.price._currency);
        }
      } else {
