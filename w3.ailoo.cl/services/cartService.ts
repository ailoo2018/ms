...
export async function findCart(wuid, domainId) {
    ...
    await initializeCart(cart, domainId);
    ...
    for (const cartItem of cart.items) {
        ...
        // PACK
            ...
            if (cartItem.type === CartItemType.Product) {
                const product = await productRepos.findProductByProductItem(cartItem.product.productItemId, domainId)
                const priceComp = await productService.getPrice(product, product.productItems[0].id, SaleType.Internet)
                let price = priceComp.getPrice()
                ...
                const {discounts, rule} = await cartService.analyze(saleContext, cartItem.packId, domainId) as any
        ...
        total = cart.items.filter(item => item).reduce((sum, it) => sum + (it.price * it.quantity), 0)
